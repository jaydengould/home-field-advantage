...
table and must travel with it: **NFL's +0.046 depends on a single control season** (drop 2018
and it falls to +0.010), and **MLB's treatment is not policy-identified at all** (74.7% of its
control games fall inside the treated dose range, vs 0.1–0.5% for the other three).

**The real finding is a ceiling, not a null.** In **seven of eight** sport × outcome cells the
...
- **More data would not fix it.** With an infinite control panel, two variance components still
  ...
  variation in home advantage. NFL's between-season variation is **1.73 margin points — larger
  than its entire 1.75-point home advantage.** Ordinary year-to-year noise exceeds the COVID signal.
...
answer** — and where a published estimate converts to our units (NBA), our interval contains it.

⚠️ **Two different power ratios appear in this project and are not interchangeable:** the
seven-of-eight figure above is per unit `crowd_pct` (`meta_cross_sport.csv`); the noise-floor
...
- ✅ **Pre-write-up audit** — the analysis was audited *before* drafting, on the reasoning that a
  ...
  - **MLB is not running the same experiment as the other three** (`dose_overlap.csv`). The share
    of control-season games falling inside the treated dose range is **NFL 0.08%, NBA 0.36%, NHL
    0.53% — and MLB 74.7%**. For three sports the treatment is a near-disjoint empty-vs-full
    ...
    cleaner than NFL. Root cause: the design specifies the treatment as identified off *policy
  ...
  - **The NFL headline rests on one control season** (`leave_one_season_out.csv`). Dropping 2018
    alone moves NFL win% by **−0.99 SE** (+0.046 → +0.010) and margin by −0.90 SE (+1.71 → +0.68);
    no other season moves either by more than 0.57 SE. MLB and NHL are stable. Read with both
...
- `results/{figures,tables}/` — generated outputs (21 tables, 3 figures)
